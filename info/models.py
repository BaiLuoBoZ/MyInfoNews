...
class User(BaseModel, db.Model):
    ...
    like_comments = db.relationship("Comment", secondary="info_comment_like", lazy="dynamic")  # 用户点赞的所有评论
    ...
    @property
    def password(self):
        raise AttributeError("该属性是计算属性，不能直接取值！")
...
class News(BaseModel, db.Model):
    ...
    def to_dict(self):
        resp_dict = {
            ...
            "index_image_url": self.index_image_url,
            "author": self.user.to_dict() if self.user else None  # 获取新闻作者
        }
        return resp_dict
